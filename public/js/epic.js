angular
    ...
    .controller('personalRecordCtrl', ['$scope', '$http', function($scope, $http) {
        ...
        $http.get(cmpt + '/rank/my/').success(function(json) {
            if (json.isSuccess) {
                if (!json.result.ranks) {
                    json.result.ranks = [];
                }
                json.result.ranks.forEach(function(series) {
                    if (series.team) {
                        $scope.fixTeam(series.team);
                    }
                    json.result.joins.series.forEach(function(join) {
                        if (join.related_id === series.series_id) {
                            series.joins = join;
                        }
                    });
                    if (series.epics) {
                        series.epics.forEach(function(epic) {
                            ...
                            json.result.joins.epics.forEach(function(join) {
                                if (join.related_id === epic.epic_id) {
                                    epic.joins = join;
                                }
                            });
                        });
                ...
                $scope.records = json.result.ranks;
            }
        });
    }])
    .controller('gameCertificateCtrl', ['$scope', '$http', function($scope, $http) {
        ...
        function generateCert() {
            $http.post(cmpt + '/api/cert/generate', {
                related_id: $scope.epic_id,
